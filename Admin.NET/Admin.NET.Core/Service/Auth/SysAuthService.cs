using Admin.NET.Core.Service.Auth.Dto;
...
public class SysAuthService : IDynamicApiController, ITransient
{
    ...
    public async Task<LoginOutput> Login([Required] LoginInput input)
    {
        ...
        {
            var pd = MD5Encryption.Encrypt(input.Password);
            if (user.Password != pd)
    ...
    /// <summary>
    /// 通过过期token和刷新token获取新token
    /// </summary>
    /// <returns></returns>
    [DisplayName("通过过期token和刷新token获取新token")]
    public string TokenByRefresh(TokenInput input)
    {
        return JWTEncryption.Exchange(input.Token,input.RefreshToken);
    }
