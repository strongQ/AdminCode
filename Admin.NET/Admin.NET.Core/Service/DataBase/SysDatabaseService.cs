...
public class SysDatabaseService : IDynamicApiController, ITransient
{
    ...
    {
        var db = _db.AsTenant().GetConnectionScope(input.ConfigId);
        if(input.OldTableName!=input.TableName)
    ...
    [DisplayName("创建实体")]
    public string CreateEntity(CreateEntityInput input)
    {
        ...
        if (input.Position=="Admin.NET.Application")
        {
    ...
    /// </summary>
    /// <param name="input"></param>
    [ApiDescriptionSettings(Name = "CreateRazor"), HttpPost]
    [DisplayName("创建Razor")]
    public List<string> CreateRazor(CreateEntityInput input)
    {
        input.Position = string.IsNullOrWhiteSpace(input.Position) ? "Admin.NET.Application" : input.Position;
        input.EntityName = input.EntityName;
        string[] dbColumnNames = _codeGenOptions.EntityBaseColumn[input.BaseClassName];
        ...
        datas.Add(tResult);
        var razorPaths = GetRazorTemplatePath();
        foreach(var razorPath in razorPaths)
        {
            var content = File.ReadAllText(razorPath);
            var razorResult = _viewEngine.RunCompileFromCached(content, new
            {
               
                input.EntityName,
               
            });
            datas.Add(razorResult);
        }
    ...
    /// </summary>
    /// <returns></returns>
    private static List<string> GetRazorTemplatePath()
    {
        List<string> result = new List<string>();
        var templatePath = App.WebHostEnvironment.WebRootPath + @"\Template\";
        result.Add(Path.Combine(templatePath, "Page.razor.vm"));
        result.Add(Path.Combine(templatePath, "Page.razor.cs.vm"));
