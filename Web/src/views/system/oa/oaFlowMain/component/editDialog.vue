...
<script lang="ts" setup>
	...
	import { addOaFlowMain, updateOaFlowMain } from "/@/api/system/oaFlowMain";
	//父级传递来的参数
  ...
  protd: [{required: true, message: '请输入全局流程模板ID！', trigger: 'blur',},],
  zform: [{required: true, message: '请输入表单数据！', trigger: 'blur',},],
  temid: [{required: true, message: '请输入全局流程模板ID！', trigger: 'blur',},],
  ...
  ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
    if (isValid) {
      ...
      if (ruleForm.value.id != undefined && ruleForm.value.id > 0) {
        await updateOaFlowMain(values);
      } else {
        await addOaFlowMain(values);
      }
      closeDialog();
    } else {
      ElMessage({
